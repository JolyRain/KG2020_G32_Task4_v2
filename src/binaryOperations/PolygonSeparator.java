...
public class PolygonSeparator {
    ...
    public List<Polygon> separate(Polygon mainPolygon, Polygon intersectPolygon) {
        ...
        if (innerPoints.size() == 1 && intersectPoints.size() == 1) aCase = Case.ONE_INNER;
        if (intersectPoints.size() == 2 && innerPoints.size() == 0) aCase = Case.ALL_OUTSIDE;
        if (aCase == null) {
            return null;
        }
        switch (aCase) {
            case ALL_INNERS:
                return allInners(mainPolygon);
            case ONE_INNER:
                return oneInner(mainPolygon);
            case ALL_OUTSIDE:
    ...
    private List<Polygon> allInners(Polygon mainPolygon) {

        if (pointNewPolygon(mainPolygon) == null) return null;
        Vector3 newPointMain = pointNewPolygon(mainPolygon);
        ...
        if (!mainPolygon.getPoint1().equals(newPointMain)) {
            ...
            mainPoint2 = !mainPolygon.getPoint2().equals(newPointMain) ? mainPolygon.getPoint2() : mainPolygon.getPoint3();  //чет говнокод прям
    ...
    private List<Polygon> oneInner(Polygon mainPolygon) {

            Vector3 newPoint1 = innerPoints.get(0);
            Vector3 intersectPoint = intersectPoints.get(0);

            Vector3 newPointMain = pointNewPolygon(mainPolygon); //возможный проблемный момент (неправильно определяется точка)
    ...
    public List<Vector3> intersectPoints(Polygon mainPolygon, Polygon intersectPolygon) {
        ...
        if (innerPoints.size() == 1 && intersectPoints.size() == 2) {
            intersectPoints.remove(farPoint(innerPoints.get(0), intersectPoints));
        }
    ...
    private Vector3 pointNewPolygon(Polygon polygon) {
        List<Line> lines = lines(polygon);

        switch (aCase) {
            case ALL_INNERS:
                Vector3 intersect = innerPoints.get(0).minus(innerPoints.get(1)); //убрать хардкод

                float minAngle = Float.MAX_VALUE;
                Line minLine = lines.get(0);

                for (Line line : lines) {
                    Vector3 vector = line.getP2().minus(line.getP1());
                    float angle = intersect.angle(vector);
                    if (Math.abs(angle) < minAngle) {   //без abs вроде работало
                        minLine = line;
                        minAngle = angle;
                    }
                }
                for (Vector3 point : polygon.getBorder().getPoints()) {
                    if (!point.equals(minLine.getP1()) && !point.equals(minLine.getP2())) {
                        return point;
                    }
                }
            case ONE_INNER:
                Vector3 intersectOut = innerPoints.get(0).minus(intersectPoints.get(0)); //убрать хардкод
                float minAngleOut = Float.MAX_VALUE;
                Line minLineOut = lines.get(0);

                for (Line line : lines) {
                    Vector3 vector = line.getP2().minus(line.getP1());
                    float angle = intersectOut.angle(vector);
                    if (Math.abs(angle) < minAngleOut) {   //без abs вроде работало
                        minLineOut = line;
                        minAngleOut = angle;
                    }
                }
                for (Vector3 point : polygon.getBorder().getPoints()) {
                    if (!point.equals(minLineOut.getP1()) && !point.equals(minLineOut.getP2())) {
                        return point;
                    }
                }
        }
    ...
    private List<Line> lines(Polygon polygon) {
